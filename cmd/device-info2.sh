...
# Mendapatkan alamat IP modem
if [[ ! -z "$getIP" ]]; then
  ip=$getIP
elif [[ ! -z "$(route -n|awk '{print $2}'|grep 192.168|head -n1)" ]]; then
  ip=$(route -n|awk '{print $2}'|grep 192.168|head -n1)
elif [[ -f "$INSDIR"/config && ! -z "$(sed -n '4p' "$INSDIR"/config)" ]]; then
  ip=$(sed -n '4p' "$INSDIR"/config)
else
  echo "Tidak dapat menentukan alamat IP modem. Periksa konfigurasi atau koneksi jaringan."
  exit 1
fi

# Menjalankan file Python dan menangkap output JSON
json_output=$( (python $INSDIR/commands/device_info.py --username $getUsrMdm --password $getUsrPswd http://$ip & pid=$!; (sleep 5; kill $pid) & wait $pid) 2>/dev/null )
